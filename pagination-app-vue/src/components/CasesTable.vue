  ...
  <div id="cases-table">
    <table>
      ...
      <tbody>
        <tr v-for="pages in display" :key="pages.id">
          ...
          <td>{{pages.doctor}}</td>
          <td>
            <div v-if="pages.status=='Closed'">
              <i class="greenTag"></i>
            </div>
            <div v-else-if="pages.status=='Open'">
              <i class="yellowTag"></i>
            </div>
            <div v-else>
              <i class="redTag"></i>
    ...
    <div class="pagination">
      <div style="float:left">
        <button v-if="page!= 0" class="previous" @click="page--">
          <i class="material-icons" style="font-size: 1.5rem;">navigate_before</i>
        </button>
        <button v-else class="previous" disabled>
          <i class="material-icons" style="font-size: 1.5rem;">navigate_before</i>
        </button>
      </div>
      <button class="pgBlock" v-on:click="page=0">1</button>
      <button class="pgBlock" v-on:click="page=1">
        <span v-if="this.page==0 || this.page== totalPages-1">2</span>
        <span v-else>...</span>
      </button>
      
      <button class="pgBlock">
        <span v-if="this.page==0 || this.page== totalPages-1">...</span>
      ...
      <div style="float:left">
        ...
        <button v-else class="next" disabled>
          <i class="material-icons">navigate_next</i>
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    totalPages() {
      return Math.ceil(this.cases.length / this.perPage);
    }
  }
};
</script>
...
td {
  ...
  text-align: left;
  padding: 0.75rem 0rem 0.75rem 4rem;
}
